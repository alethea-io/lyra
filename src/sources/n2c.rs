...
async fn intersect_from_breadcrumbs(
    ...
    let (intersect, _) = peer
        .chainsync()
        .find_intersect(breadcrumbs.to_points())
        .await
        .or_restart()?;
    ...
    async fn process_next(
        ...
        match next {
            NextResponse::RollForward(cbor, tip) => {
                ...
                let evt = ChainEvent::Apply(point.clone(), Record::RawBlockPayload(cbor.to_vec()));
...
impl gasket::framework::Worker<Stage> for Worker {
    async fn bootstrap(stage: &Stage) -> Result<Self, WorkerError> {
        ...
        if stage.cursor.is_empty() {
            ...
            intersect_from_breadcrumbs(&mut peer_session, &stage.cursor).await?;
        }
